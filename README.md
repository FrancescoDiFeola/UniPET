...
(a) A segmentation model $S$ is applied to each CT scan $\bm{x}$ to produce a multi-label anatomical mask $\bm{M}$, which is used to extract region-specific subvolumes from paired CT and PET scans. 
...
A generator $G = G_e \circ G_d$ synthesizes PET volumes, while an auxiliary classifier $C$ predicts anatomical regions from the latent representation $\bm{z}$. 
