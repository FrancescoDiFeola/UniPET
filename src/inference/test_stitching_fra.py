...
for i, data in enumerate(data_loader):
    ...
    # Calcolo metriche
    mae_value = np.mean(np.abs(data['B'].as_tensor().permute(0, 2, 3, 4, 1).numpy().squeeze() - complete_generated_image.squeeze()))
    psnr_value = psnr(data['B'].as_tensor().permute(0, 2, 3, 4, 1).numpy(), complete_generated_image)
    ssim_value, _ = ssim( data['B'].as_tensor().permute(0, 2, 3, 4, 1).numpy().squeeze(), complete_generated_image.squeeze(), full=True,data_range=1.0)

    print(f'Progress: Paziente {i + 1}/{dataset_size} - MAE: {mae_value} - PSNR: {psnr_value} - SSIM: {ssim_value}')
    

    mae_per_patient.append(mae_value)
    psnr_per_patient.append(psnr_value)
    ssim_per_patient.append(ssim_value)
    mean_mae_per_patient = np.mean(mae_per_patient)
    std_mae_per_patient = np.std(mae_per_patient)
    mean_psnr_per_patient = np.mean(psnr_per_patient)
    std_psnr_per_patient = np.std(psnr_per_patient)
    mean_ssim_per_patient = np.mean(ssim_per_patient)
    std_ssim_per_patient = np.std(ssim_per_patient)
...
print(mae_per_patient)
print(psnr_per_patient)
print(ssim_per_patient)
print("Mean MAE totale: ", mean_mae_per_patient)
print("Deviazione standard totale MAE: ", std_mae_per_patient)
print("Mean PSNR totale: ", mean_psnr_per_patient)
print("Deviazione standard totale PSNR: ", std_psnr_per_patient)
print("Mean SSIM totale: ", mean_ssim_per_patient)
print("Deviazione standard totale SSIM: ", std_ssim_per_patient)

# Save results to CSV
out_path = opt.out_path
os.makedirs(out_path, exist_ok=True)

csv_filename = os.path.join(out_path, "patient_metrics_.csv")
df_metrics = pd.DataFrame({
    "Patient_ID": list(range(1, 141 + 1)),
    "MAE": mae_per_patient,
    "PSNR": psnr_per_patient,
    "SSIM": ssim_per_patient
})
df_metrics.to_csv(csv_filename, index=True)
